let DiffContext = require('../contexts/diff').DiffContext
let PatchContext = require('../contexts/patch').PatchContext
let ReverseContext = require('../contexts/reverse').ReverseContext
...
// A value exists but not at the same position
...
// Match a value given two arrays and two indexes
...
let diffFilter = function arraysDiffFilter(context) {
...
let patchFilter = function nestedPatchFilter(context) {
  if (!context.nested) {
    return
  }
...
let collectChildrenPatchFilter = function collectChildrenPatchFilter(context) {
...
let reverseFilter = function arraysReverseFilter(context) {
  if (!context.nested) {
...
let reverseArrayDeltaIndex = function(delta, index, itemDelta) {
...
let collectChildrenReverseFilter = function collectChildrenReverseFilter(context) {
  if (!context || !context.children) {
    return
  }
  if (context.delta._t !== 'a') {
